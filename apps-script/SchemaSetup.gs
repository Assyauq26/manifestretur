/**
 * DATABASE SCHEMA - CANONICAL MAIN VERSION
 * Handover keeps receiver name and signature metadata.
 * Migration only adds missing columns; existing data is not deleted.
 */
...
function migrateHandoverSchema() {
  ...
  sheet.setFrozenRows(1);
  SpreadsheetApp.flush();
  Logger.log(added.length ? 'HANDOVER schema updated: ' + added.join(', ') : 'HANDOVER schema sudah sesuai.');
  return added;
}

...
function handleGetSellers() {
  ...
  const idx = indexHeaders_(headers, ['seller_id', 'seller_name', 'receiver_name', 'phone', 'status'], 'SELLER_MASTER');
  const sellers = [];
...
function handleGenerateManifest(requestData) {
  ...
  try {
    ...
    if (!['1', '2', '3'].includes(shift)) return createErrorResponse('Shift tidak valid.');
    if (!sellerId) return createErrorResponse('Seller wajib dipilih.');
    ...
    const sellerIdx = indexHeaders_(sellerHeaders, ['seller_id', 'seller_name', 'receiver_name', 'phone', 'status'], 'SELLER_MASTER');
    ...
    const awbIdx = indexHeaders_(awbHeaders, ['manifest_awb_id', 'manifest_id', 'awb', 'sequence', 'input_method', 'scanned_at', 'scanned_by', 'status'], 'MANIFEST_AWB');
    ...
    return createJsonResponse({
      ...
      data: { manifestId: manifestId, manifestNumber: manifestNumber, pdfUrl: pdfResult.url, status: PHASE7_READY_STATUS }
    });
  } catch (error) {
...
function indexHeaders_(headers, required, sheetName) {
  ...
  required.forEach(function(key) {
    if (map[key] === undefined) throw new Error('Kolom ' + sheetName + ' wajib: ' + key);
  });
  return map;
}

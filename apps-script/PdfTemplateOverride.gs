 ...
 *
 * Bahasa pada PDF dibuat hanya dalam Bahasa Indonesia.
 *
...
function buildManifestPageHtml_(params) {
  const {
    manifestNumber, dateStr, safeDropPoint, safeSprinter, safeSprinterPhone,
    safeSeller, finalReceiver, safeSellerPhone, awbs, startIndex, rowCount,
    pageNumber, totalPages, isLastManifestPage, handoverCompleted, signatureHtml
  } = params;
...
<div class="manifest-page${lastClass}">
  <div class="top">
    <div class="system">System-Based / GunScanner</div>
    <img class="logo-image" src="${params.logoDataUri}" alt="J&T Express" />
    <div class="date">HARI/TANGGAL: ${dateStr}</div>
    <div class="page-number">Halaman ${pageNumber}/${totalPages}</div>
  </div>

  <div class="title">FORM RETUR PENGEMBALIAN BARANG SELLER</div>
  <div class="subtitle">FORM SERAH TERIMA BARANG KEPADA SELLER</div>

  <table class="info-table">
    <tr>
      <td class="section" colspan="2">DATA DROPPOINT</td>
      <td class="section" colspan="2">DATA SELLER</td>
    </tr>
    <tr>
      <td class="label">DROPPOINT</td>
      <td class="value">${safeDropPoint}</td>
      <td class="label">NAMA SELLER DI SISTEM</td>
      <td class="value">${safeSeller}</td>
    </tr>
    <tr>
      <td class="label">NAMA LENGKAP SPRINTER</td>
    ...
    <tr>
      <td class="label">NO. HP SPRINTER</td>
      <td class="value">${safeSprinterPhone}</td>
      <td class="label">NO. HP SELLER</td>
  ...
  <table class="awb-table">
    <thead><tr>
      ...
      <th class="awb-head">AWB</th><th class="awb-head">AWB</th>
      <th class="awb-head">AWB</th><th class="awb-head">AWB</th>
      <th class="awb-head">AWB</th>
    </tr></thead>
    <tbody>${buildManifestAwbGridHtml_(awbs, startIndex, rowCount)}</tbody>
  </table>

  <div class="total">${pageSummary}</div>

  <div class="footer">
    <div>Diatas adalah data nomor AWB yang telah kami serahkan kepada PIC Seller.</div>
    <div class="line">Form ini sebagai bukti serah terima barang yang telah diretur oleh Sprinter DP.</div>
...
function generatePdfDrive(manifestNumber, user, sellerName, receiverName, sellerPhone, awbs, options) {
  ...
  // Kapasitas formulir: 5 kolom x 35 baris = 175 AWB per halaman.
  ...
  for (let page = 0; page < totalPages; page++) {
    ...
    manifestPages.push(buildManifestPageHtml_({
      manifestNumber, dateStr, safeDropPoint, safeSprinter, safeSprinterPhone,
      safeSeller, finalReceiver, safeSellerPhone, awbs, startIndex, rowCount,
      pageNumber: page + 1, totalPages, isLastManifestPage,
      handoverCompleted, signatureHtml, logoDataUri
    }));
  }
  ...
  if (options.handoverPhotoBase64) {
    ...
    const safeHandoverBy = escapePdfHtml_(
      options.handoverBy ? getSprinterDisplayNameForPdf_(options.handoverBy) : sprinterName
    );
...
<html><head><meta charset="UTF-8"><style>
  ...
  * { box-sizing:border-box; }
  ...
  .manifest-page { width:100%; min-height:278mm; page-break-after:always; break-after:page; page-break-inside:avoid; break-inside:avoid; overflow:hidden; }
  ...
  .handover-page { width:100%; min-height:278mm; page-break-before:always; break-before:page; text-align:center; padding-top:2mm; page-break-inside:avoid; break-inside:avoid; }

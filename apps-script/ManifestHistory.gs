...
function handleGetManifests_(requestData) {
  ...
  for (let r = values.length - 1; r >= 1; r--) {
    ...
    const allowed = isAdmin || sameValueForHandover_(row[idx.sprinter_id], user.sprinter_id) || sameValueForHandover_(row[idx.drop_point_id], user.drop_point_id);
    if (!allowed) continue;
  ...
  return createJsonResponse({ success: true, data: rows.slice(offset, offset + limit), meta: { total, offset, limit } });
}

function handleGetHistory_(requestData) {
  const user = getSessionUserForHandover_(requestData && requestData.userToken);
  const manifestSheet = getRequiredSheetForHandover_('MANIFEST');
  const values = manifestSheet.getDataRange().getValues();
  if (values.length < 2) return createJsonResponse({ success: true, data: [] });
  ...
  return createJsonResponse({ success: true, data: result.slice(offset, offset + limit), meta: { total: result.length, offset, limit } });
}

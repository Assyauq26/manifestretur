...
function SignaturePad({ value, onChange, disabled }) {
  ...
  const setupCanvas = useCallback(() => {
    ...
    if (!canvas) return;
    // Keep a stable drawing buffer and map pointer coordinates from CSS pixels
    // into that buffer. This avoids the offset that occurs when CSS size,
    // devicePixelRatio and browser zoom differ.
    canvas.width = 1200;
    canvas.height = 480;
...
export default function HandoverPageV3() {
  ...
  const submitHandover = async (event) => {
    ...
    if (!photoPreview) return setErrorMessage('Foto bukti serah terima wajib diambil.');
    if (!receiverName.trim()) return setErrorMessage('Nama penerima wajib diisi.');
    if (!signatureData) return setErrorMessage('Tanda tangan PIC Seller wajib diisi.');
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20 max-w-md mx-auto shadow-sm relative">
      <header className="bg-white px-5 py-4 sticky top-0 z-10 border-b border-gray-100">
        ...
        <p className="text-xs text-gray-500">Foto dan TTD penerima akan masuk ke PDF final manifest</p>
      </header>
        ...
        <div className="fixed inset-0 z-[400] bg-black/50 flex items-end justify-center" role="dialog" aria-modal="true">
          <div className="w-full max-w-md bg-white rounded-t-3xl p-5 max-h-[92vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-5">
              <div><h2 className="text-xl font-bold text-gray-900">Serah Terima</h2><p className="text-xs text-gray-500 mt-1">{getManifestId(selectedManifest)}</p></div>
            ...
            <form onSubmit={submitHandover} className="space-y-5">
              ...
              <div>
                ...
                <input value={receiverName} onChange={(event) => setReceiverName(event.target.value)} type="text" autoComplete="name" placeholder="Masukkan nama lengkap penerima" className="w-full rounded-2xl border border-gray-200 bg-white px-4 py-3.5 text-sm outline-none focus:border-[#D71920]" disabled={isSubmitting} />
              </div>
              ...
              <div>
                ...
                {photoPreview ? (
                  ...
                  <button type="button" onClick={() => fileInputRef.current?.click()} className="w-full rounded-2xl border-2 border-dashed border-gray-200 bg-gray-50 p-8 flex flex-col items-center justify-center text-gray-500">
                    ...
                    <span className="text-xs mt-1">Foto akan dimasukkan ke halaman terakhir PDF</span>
                  </button>
                )}
              ...
              <div className="rounded-2xl border border-blue-100 bg-blue-50 p-4 text-xs text-blue-800">
                <strong>Catatan:</strong> Nama penerima dan TTD PIC Seller dicetak pada halaman pertama PDF final. Foto serah terima ditempatkan di halaman terakhir.
              </div>
        ...
        <div className="fixed inset-0 z-[800] bg-black/55 flex items-center justify-center p-5" role="dialog" aria-modal="true">
          <div className="w-full max-w-sm bg-white rounded-3xl p-6 shadow-2xl">
            ...
            <div className="space-y-3 mt-6">
              ...
              <button type="button" onClick={goHomeAfterSuccess} className="w-full py-3.5 rounded-2xl bg-[#D71920] text-white font-bold">Ke Beranda</button>
              <button type="button" onClick={closeSuccessModal} className="w-full py-3.5 rounded-2xl bg-gray-100 text-gray-800 font-bold">Tetap di Hand Over</button>
            </div>
          </div>
        </div>
      )}
